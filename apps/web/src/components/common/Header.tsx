    ...
    <header className="sticky top-0 z-40 w-full border-b border-[#1a1918]/20 bg-[#fdfaf6]/90 backdrop-blur-md">
      <div className="container mx-auto px-4 lg:px-8 h-20 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 w-1/3 editorial-caption font-semibold">
          <Link href="/" className={clsx("transition-colors", isActive("/") ? "text-[#1a1918] border-b border-[#1a1918]" : "text-[#4a4845] hover:text-[#1a1918]")}>
            Issues / 首页
          </Link>
          <Link href="/discover" className={clsx("transition-colors", isActive("/discover") ? "text-[#1a1918] border-b border-[#1a1918]" : "text-[#4a4845] hover:text-[#1a1918]")}>
            Curated / 发现
          </Link>
          <Link href="/pricing" className={clsx("transition-colors flex items-center gap-1", isActive("/pricing") ? "text-[#c44d36] border-b border-[#c44d36]" : "text-[#4a4845] hover:text-[#c44d36]")}>
            Subscribe / 订阅
          </Link>
        </nav>
        ...
        <div className="flex items-center justify-end gap-5 w-1/3">
          ...
          <button onClick={() => router.push('/upload')} className="hidden sm:flex items-center justify-center border border-[#1a1918] bg-transparent hover:bg-[#1a1918] hover:text-[#fdfaf6] text-[#1a1918] px-4 h-9 text-xs font-bold tracking-widest uppercase transition-colors gap-1.5">
            <Plus className="w-3.5 h-3.5" />
            Publish
          </button>

          <Link href={`/profile/${CURRENT_USER.id}`} className="relative block border border-transparent hover:border-[#1a1918] p-0.5 transition-colors">
            <img
