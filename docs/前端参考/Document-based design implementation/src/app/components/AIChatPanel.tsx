...
export function AIChatPanel({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  const [messages, setMessages] = useState<{role: 'ai'|'user', content: string}[]>([
    {
      role: 'ai',
      content: 'Welcome to the Editorial Desk. I can assist you with metadata generation, critique your compositions, or refine your synopsis. How may I assist your publication today?'
    }
  ]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      setMessages([...newMessages, { 
        role: 'ai', 
        content: 'I understand your vision. Let me search the archives for related thematic references. Might I suggest emphasizing the high-contrast elements in your prompt for a more dramatic impact?' 
      }]);
    }, 1000);
  };
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-[#1a1918]/20 bg-texture-light">
              <div className="flex items-center gap-4">
                <span className="editorial-title text-2xl tracking-tighter italic">AI Assistant</span>
                <span className="editorial-caption text-[#c44d36] font-bold border-l border-[#1a1918]/20 pl-4">ONLINE</span>
              </div>
              <button 
                onClick={onClose}
